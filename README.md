...
- **`add_place` and `add_note` are idempotent.** Re-adding the same place to the same section at the same start time, or the same note text to the same section, is now a no-op that says where it already is instead of creating a duplicate. A different time or a different day is a real second visit and still inserts.
- **Stale reads no longer cause duplicate writes.** `get()` never checked the cached snapshot against the live document, so a successful write could be followed by a read that did not show it — and an agent that concludes its write failed writes again, which is how duplicate places and notes appear. Reads now compare the cached version against the ShareDB client's and refetch on mismatch; an unknown subtype op throws (tripping the existing resubscribe path) instead of being skipped while the new version is recorded anyway; and a second copy of one of our own op frames is no longer applied a second time, which duplicated a block on any `li` insert.
- **The realtime transport can no longer kill the server process.** A handshake send from a stale socket threw out of a `ws` event listener, and a failed resubscribe after reconnect was an unhandled rejection — either one exited the process, which a stdio MCP client does not restart. Listeners are now bound to their own socket and cannot throw into the event loop, reconnect shares the single in-flight connect attempt, transient handshake failures get two bounded retries while auth and rate-limit failures fail fast, and dropped-socket / timeout / expired-cookie / rate-limited are now distinct errors instead of one opaque failure.
